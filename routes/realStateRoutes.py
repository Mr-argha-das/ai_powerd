...
from models.realstate import ConversationRealState, MessageRealState, UserRealState
...
router = APIRouter(tags=["Real State"])

@router.get("/realstate/chats/inbox/{user_id}")
async def get_inbox(user_id: str, db: Session = Depends(get_db)):
    selfUser = db.query(UserRealState).get(int(user_id))
    conversations = db.query(ConversationRealState).filter(
        (ConversationRealState.user1_id == user_id) | (ConversationRealState.user2_id == user_id)
    ).all()
    inbox_list = []
    ...
    for convo in conversations:
        ...
        user = db.query(UserRealState).get(other_user_id)
        print(user)

        last_msg = db.query(MessageRealState).get(convo.last_message_id)
        if last_msg:
...
@router.get("/realstate/chats/history/{user1}/{user2}")
async def get_chat_history(user1: str, user2: str, db: Session = Depends(get_db)):
    messages = db.query(MessageRealState).filter(
        ((MessageRealState.sender_id == user1) & (MessageRealState.receiver_id == user2)) |
        ((MessageRealState.sender_id == user2) & (MessageRealState.receiver_id == user1))
    ).order_by(MessageRealState.timestamp.asc()).all()
...
@router.post("/realstate/chats/mark_seen/{conversation_id}/{user_id}")
async def mark_messages_as_seen(conversation_id: str, user_id: str, db: Session = Depends(get_db)):
    conversation = db.query(ConversationRealState).get(conversation_id)

    if not conversation or (int(user_id) not in [conversation.user1_id, conversation.user2_id]):
        raise HTTPException(status_code=403, detail="Not authorized to access this chat.")

    db.query(MessageRealState).filter(
        MessageRealState.receiver_id == user_id,
        MessageRealState.sender_id.in_([conversation.user1_id, conversation.user2_id]),
        MessageRealState.is_read == False
    ).update({"is_read": True})
    db.commit()

    last_msg = db.query(MessageRealState).get(conversation.last_message_id)
...
@router.get("/realstate/matrimonies")
def get_all_customers(db: Session = Depends(get_db)):
    customers = db.query(UserRealState).all()
    return {
...
# ============================================================
def _safe_user_field(user: Optional[UserRealState], field_name: str, fallback=None):
    if not user:
        return fallback
...
def build_inbox(db: Session, user_id: str):
    ...
    conversations = db.query(ConversationRealState).filter(
        (ConversationRealState.user1_id == uid_int) |
        (ConversationRealState.user2_id == uid_int)
    ).all()
    ...
    for convo in conversations:
        ...
        other_user = db.query(UserRealState).get(other_id)
        last_msg = None
...
async def save_and_push_message(sender_id, receiver_id, message, db: Session):
    ...
    convo = db.query(ConversationRealState).filter(
        ((ConversationRealState.user1_id == s_id) & (ConversationRealState.user2_id == r_id)) |
        ((ConversationRealState.user1_id == r_id) & (ConversationRealState.user2_id == s_id))
    ).first()

    if not convo:
        convo = ConversationRealState(
            user1_id=s_id,
            user2_id=r_id,
...
# ============================================================
@router.websocket("/realstate/chat/ws/{user_id}")
...
# ============================================================
@router.websocket("/realstate/inbox/ws/{user_id}")
